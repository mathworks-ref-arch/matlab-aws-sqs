...
if ~isdeployed()
    ...
    iDisplayBanner('MATLAB Interface for Amazon SQS');
    ...
    %% Add the SQS interface to the path
    iAddSQS(here);
...
if ~exist(commonDir, 'dir')
    ...
    error('AWS:SQS',['Could not locate common utilities at: ',commonDir]);
end
...
if ~exist(commonJarPath,'file') && ~exist(jarPath,'file')
    error('AWS:SQS','Could not locate jar file at: %s or %s',jarPath, commonJarPath);
end
...
rootDirs={fullfile(sqsDir,'app'),true;...
    fullfile(sqsDir,'lib'),false;...
    fullfile(sqsDir,'sys','modules'),true;...
    fullfile(sqsDir,'public'),true;...
    };
