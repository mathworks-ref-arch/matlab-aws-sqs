...
write(logObj,'debug','Initializing SQS client');
...
%% check if SQS is available in the given region
availableRegions = RegionUtils.getRegionsForService('sqs');
if  ~contains(char(availableRegions), char(awsRegion.getName))
    write(logObj,'error',['Region: ',awsRegion.getName,' does not support SQS']);
end


%% build a handle to the SQS client
builderH = AmazonSQSClientBuilder.standard().withCredentials(awsCredentials).withClientConfiguration(obj.clientConfiguration.Handle);
...
% configure the Endpoint if set - TODO check this is supported for SQS builder
if ~isempty(obj.endpointURI.Host)
    % write(logObj,'debug','Configuring SQS Endpoint');
